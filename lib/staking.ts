...
export async function withdrawStake(amount: bigint | number): Promise<`0x${string}`> {
  if (!isStakingConfigured()) throw new Error("Staking not configured");
  const acc = getAccount(wagmiConfig);
  if (!acc?.address) throw new Error('No active account');
...
export async function claimStakeReward(): Promise<`0x${string}`> {
  if (!isStakingConfigured()) throw new Error("Staking not configured");
  const acc = getAccount(wagmiConfig);
  if (!acc?.address) throw new Error('No active account');
...
export async function exitStake(): Promise<`0x${string}`> {
  if (!isStakingConfigured()) throw new Error("Staking not configured");
  const acc = getAccount(wagmiConfig);
  if (!acc?.address) throw new Error('No active account');
