...
export default function InfoPage() {
  ...
  const buildCastMessage = () => {
    ...
    const correctTrim = (form.correct_answer || "").trim();
    const entries = [
      ...incorrect.map((o) => ({ text: o, isCorrect: false })),
      { text: correctTrim, isCorrect: true },
    ].filter((e) => e.text && e.text.length);

    // Shuffle entries so the correct answer can appear at different letters each time
    ...
    if (correctIndex !== -1) {
      ...
      parts.push(`Correct: ${correctLetter}`);
    }
